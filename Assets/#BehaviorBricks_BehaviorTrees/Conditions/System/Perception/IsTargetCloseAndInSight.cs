    ...
    public class IsTargetCloseAndInSight : GOCondition
    {
		...
		public override bool Check()
        {
            ...
            if (dir.sqrMagnitude > closeDistance * closeDistance)
                return false;
            RaycastHit hit;
            Debug.DrawRay(gameObject.transform.position + new Vector3(0, 0.1f, 0), dir);
            if (Physics.Raycast(gameObject.transform.position + new Vector3(0, 0.1f, 0), dir, out hit))
            {
                return hit.collider.gameObject.tag.Equals(target.tag) && Vector3.Angle(dir, gameObject.transform.forward) < angle * 0.5f;
            }
            return false;
		}
    }
